...
namespace Sensemaking.Specs
{
    ...
    public partial class AsyncActionsSpecs : Specification
    {
        ...
        public void faulty_action_reports_failure_after_last_retry_attempt()
        {
            scenario(() =>
            {
                Given(an_async_action_that_constantly_fails);
                And(with_retry_that_handles_exceptions_for_a_period);
                When(() => trying(executing_async));
        ...
        {
            Given(a_flaky_async_action_that_may_fail);
            And(with_retry_that_handles_exceptions_for_a_period);
            When(executing_async);
